...
const OSIS_APP_BUILD = 'v23.0.0';
const OSIS_APP_VERSION_URL = './app-version.json';
const OSIS_UPDATE_CHECK_MS = 60 * 1000;

let waitingServiceWorker = null;
let pwaRegistration = null;
let reloadingForPwaUpdate = false;
let pwaHadControllerAtBoot = Boolean(navigator.serviceWorker?.controller);
let deferredAutoReloadTimer = null;
...
function isUserEditingForAutoUpdate() {
...
function reloadForLatestBuild() {
    ...
    if (isUserEditingForAutoUpdate()) {
        clearTimeout(deferredAutoReloadTimer);
        deferredAutoReloadTimer = setTimeout(reloadForLatestBuild, 5000);
        return;
    }

    reloadingForPwaUpdate = true;
    try {
        showPwaToast('Versi baru tersedia. Memuat pembaruan...', 'success');
    } catch (_) {}
...
async function checkRemoteAppVersion() {
    ...
    try {
        const separator = OSIS_APP_VERSION_URL.includes('?') ? '&' : '?';
        const response = await fetch(
            `${OSIS_APP_VERSION_URL}${separator}t=${Date.now()}`,
        ...
        if (remoteVersion && remoteVersion !== OSIS_APP_BUILD) {
            reloadForLatestBuild();
            return true;
        }
    } catch (error) {
        // Update checker tidak boleh mengganggu penggunaan website saat koneksi buruk/offline.
    }
...
async function registerPwaServiceWorker() {
    ...
    try {
        ...
        setTimeout(checkForPwaUpdate, 2500);
        setInterval(checkForPwaUpdate, OSIS_UPDATE_CHECK_MS);
    } catch(err) {
...
navigator.serviceWorker?.addEventListener('controllerchange', () => {
    if (!pwaHadControllerAtBoot) {
        pwaHadControllerAtBoot = true;
        return;
    }
    reloadForLatestBuild();
});
