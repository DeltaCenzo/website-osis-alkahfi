/* =========================================================
   OSIS AL-KAHFI — V40 GODMODE MOTION SYSTEM
...
(()=>{
  ...
  if(loader){
    ...
    const minDuration=reduced?360:(lite?1250:3000);
    const hardLimit=reduced?850:(lite?2400:4900);
    ...
    const finish=()=>{
      ...
      setTimeout(()=>{
        ...
        // Reveal hero while panels are leaving, so the school photo is visible
        // immediately through the center gap.
        setTimeout(markHeroLive,reduced?10:110);
        setTimeout(()=>loader.remove(),reduced?300:1250);
      },reduced?20:240);
    };
  ...
  if(header&&heroBg&&!lite){
    ...
    if(fine){
      header.addEventListener('pointermove',(event)=>{
        ...
        tx=Math.max(-10,Math.min(10,nx*20));
        ty=Math.max(-7,Math.min(7,ny*14));
        schedule();
      },{passive:true});
  ...
  const paintScroll=()=>{
    ...
    sections.forEach((section)=>{
      ...
      section.style.setProperty('--v40-section-shift',`${(-norm*34).toFixed(1)}px`);
    });
  };
