...
function galleryPhotoCandidates(photo) {
    const urls = [];
    const add = value => {
    ...
    // Backend V12 mengirim URL view yang sudah membawa resource key.
    add(photo?.url);
    add(photo?.downloadUrl);

    // Fallback tambahan untuk file Drive yang dibagikan via link.
    ...
    if (id) {
        ...
        const resourcePart = resourceKey
            ...
            : '';
        add(`https://drive.google.com/thumbnail?id=${encodeURIComponent(id)}&sz=w2000${resourcePart}`);
...
function galleryPhotoUrl(photo) {
    return galleryPhotoCandidates(photo)[0] || '';
}

function setGalleryImageSource(img, photo, onFinalError) {
    if (!img) return;
    const urls = Array.isArray(photo?.urls) && photo.urls.length
        ? photo.urls.filter(Boolean)
        : galleryPhotoCandidates(photo);
    let index = 0;
...
function renderPublicGallery() {
    ...
    albums.forEach((album, index) => {
        ...
        if (coverSrc) {
            ...
            setGalleryImageSource(img, cover, () => { img.style.display = 'none'; });
...
function getAlbumViewerPhotos(album) {
    ...
    return album.photos.map((p, i) => {
        const urls = galleryPhotoCandidates(p);
        return {
            src: urls[0] || '',
            urls,
...
function openAlbum(key) {
    ...
    photos.forEach((photo, i) => {
        ...
        img.decoding = 'async';
        setGalleryImageSource(img, photo, () => { img.style.opacity = '.25'; });
